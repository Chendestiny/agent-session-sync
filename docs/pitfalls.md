...
| **webui 导入徽章对 claude/opencode 失明** | 写进这两家的导入会话在 web（原 serve）卡片/列表上看不到（导入数恒 0），dsh/codex/hermes/workbuddy 却正常 | 根因：`_display_sessions` 展示口径只给四家传了 `include_imports=True`，claude 走默认 reader 且 `read_claude` 连开关都没有（无条件跳 uuid5）。修复：`read_claude(dir, include_imports=False)` 加参 + webui 补 claude/opencode 分支 |
...
| **各 agent skills 目录不统一，通用位装了别家看不见** | 朋友 WorkBuddy 跑一句话安装后找不到 skill——安装落在通用位 `~/.agents/skills/`，但 WorkBuddy 只扫 `~/.workbuddy/skills/`，claude/codex/hermes/dsh 也各守自家目录 | install 第 4 步自动桥接：在每个**检测到**的自家 skills 目录里建 junction/symlink 指回唯一源（存在才桥接；已存在实体目录则跳过并提示）；桥接后重启对应 agent |
| **install.ps1 两颗 Windows 雷** | ① 无 BOM 的 UTF-8 里写中文注释，PowerShell 5.1 按 GBK 误读直接解析报错；② `Remove-Item -Recurse` 删 junction 在 PS5.1 可能穿透删掉目标内容（唯一源被清） | ① install.ps1 保持 ASCII（中文说明写进 README/AGENTS）；② 删 junction 用 `[System.IO.Directory]::Delete($link)` 只删 reparse 点本身 |
