...
用法（退出 dsh 后）：
    python sync-finish.py            # 全部收尾
    python sync-finish.py --check    # 只看当前状态，不动任何东西

流程：
  0. 检查 dsh 已退出（未退出则每 10s 等待，可 Ctrl+C 取消后重来）
  1. prune   ：清理孤儿导入（源已删/被过滤）+ 打招呼冒烟会话 → 回收站+墓碑
  2. to-dsh  ：全源增量导入（幂等；workbuddy 已排除 playground）
...
def main():
    ...
    print("=" * 56)
    print("== [2/4] to-dsh：全源增量导入")
    print("=" * 56)

    from types import SimpleNamespace as NS
    a = NS(source="zcode,hermes,codex,workbuddy", apply=not check_only, root=root,
